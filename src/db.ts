...
export class MoviesDatabase {
  ...
  // Get all movies
  async getMovies() {
    return this.db.query('SELECT * FROM movies').all() as Movie[];
  }
  ...
  async addMovie(movie: Movie) {
    return this.db
      .query(
        `INSERT INTO movies (title, director) VALUES (?, ?) RETURNING title, director, id`
      )
  ...
  async init() {
    ...
    return this.db.run(
      `INSERT OR IGNORE INTO movies (id, title, director) VALUES (1, 'The Artist', 'Michel Hazanavicius'),(2, 'Argo', 'Ben Affleck'),(3, '12 Years a Slave', 'Steve McQueen'),(4, 'Birdman', 'Alejandro G. Inarritu'),(5, 'Spotlight', 'Tom McCarthy'),(6, 'Moonlight', 'Barry Jenkins'),(7, 'The Shape of Water', 'Guillermo del Toro'),(8, 'Green Book', 'Peter Farrelly'),(9, 'Parasite', 'Bong Joon-ho'),(10,'Nomadland','Chloe Zhao');`
    );
  }
}
